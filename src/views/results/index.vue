...
import { exportResults, getResultDetail, getConversationResultDetail, listResults, type InspectionResult } from "@/api/results";
import { groupResultsByTaskAndConversation, type TaskConversationResultSummary } from "./group-results";
...
const conversationRows = computed(() => {
  return groupResultsByTaskAndConversation(results.value);
});
async function refresh() { loading.value = true; try { const result = await listResults(filters.value, { current: page.value, size: pageSize.value }); results.value = result.records; total.value = result.total; } catch { message.error("结果加载失败"); } finally { loading.value = false; } }
